...
const parseResponse = (res: Response, path: string) => {
  ...
  try {
    ...
    if ('error' in jsonBody && ![400].includes(res.statusCode)) {
      ...
      dpst1093Hint = 'For lab10_deploy, the only acceptable status code for error cases is 400. ' +
    ...
    if (res.statusCode === 404) {
      ...
      dpst1093Hint = 'Your server has crashed. Check the terminal running the server to see the error stack trace';
    } else {
      dpst1093Hint = 'Your routes may not be returning a valid JSON response - for example, the /clear should still return an empty object `{}` instead of undefined.';
    }
  }
  const ret = {
